...
from linkml_arrays.graph_utils.graph import ObjectGraph, GraphNode
...
class GraphDeserializer:
    ...
    def deserialize(
        ...
        if not isinstance(root.obj, target_class):
            raise TypeError(
                f"Expected {target_class.__name__}, "
                f"got {type(root.obj).__name__}"
            )
    ...
    def _instantiate(
        ...
        for edge in node.outgoing:
            ...
            if child.obj is None:
                raise RuntimeError(
                    f"{child.class_name} has not yet been instantiated."
                )
